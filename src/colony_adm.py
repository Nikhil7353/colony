...
def clone():
    ...
    try:
        root_file = open(root_file_path, "a")
    finally:
        root_file.close()
...
def generate():
    ...
    # a runtime error
    if len(sys.argv) < 3:
        raise RuntimeError("No plugin file provided")
...
def build():
    ...
    # a runtime error
    if len(sys.argv) < 3:
        raise RuntimeError("No descriptor provided")
...
def deploy():
    ...
    if len(sys.argv) < 3:
        raise RuntimeError("No package provided")
...
def install():
    ...
    if len(sys.argv) < 3:
        raise RuntimeError("No name of package provided")
...
def require():
    ...
    if len(sys.argv) < 3:
        raise RuntimeError("No requirements file provided")
...
def _generate(path, build=True, delete=True):
    ...
    if is_plugin:
        descriptor_path = _generate_plugin(path)
    elif is_config:
        descriptor_path = _generate_config(path)
    else:
        raise RuntimeError("Invalid path provided for generation")
...
def _generate_plugin(path, use_path=True):
    ...
    for name in names:
        ...
        if not os.path.isdir(current):
            continue
        if name in (short_name, short_name + "_c"):
            mode = "direct"
            break
        else:
            target = name
    ...
    if mode == "direct":
        resources = _gather_direct(short_name, base_dir, name)
    elif mode == "indirect":
        resources = _gather_indirect(short_name, base_dir, target)
    else:
        resources = _gather_invalid(short_name, base_dir, name)
    ...
    try:
        descriptor_file.write(structure_s)
    finally:
...
def _build(path, short_name=True):
    ...
    if is_plugin:
        base_name = colony.to_underscore(base_name)[:-12]
    elif is_config:
        base_name = colony.to_underscore(base_name)[:-12]
    else:
        base_name = colony.to_underscore(base_name)[:-5]
...
def _deploy(path, timestamp=None):
    ...
    if type == "plugin":
        ...
        RuntimeError("invalid package type")
    ...
    if is_unicode:
        descriptor_s = descriptor_s.encode("utf-8")
    file = open(spec_path, "wb")
    try:
        file.write(descriptor_s)
    finally:
        file.close()
...
def _info(path):
    ...
    type = descriptor.get("type", "plugin")
    if type == "plugin":
        _info_plugin(descriptor)
    elif type == "config":
        _info_config(descriptor)
    else:
        RuntimeError("invalid package type")
...
def _install(name=None, id=None, version=None, upgrade=False):
    ...
    params = dict(filters=[])
    if name:
        params["filters"].append("name:equals:%s" % name)
    if id:
        params["filters"].append("identifier:equals:%s" % id)
    ...
    if not type(repo_url) in (list, tuple):
        repo_url = (("colony", repo_url),)
...
def _upgrade():
    ...
    for plugin in plugins:
        _install(plugin, upgrade=True)
    for config in configs:
        _install(config, upgrade=True)
...
def _exists(info, upgrade=False):
    ...
    data = data.decode("utf-8")
    descriptor = json.loads(data)
    if not info["version"] == descriptor["version"]:
        return False
    if info["timestamp"] > descriptor["timestamp"]:
        return False
...
def _gather_direct(short_name, base_dir, name):
    ...
    if not exists:
        root_path = os.path.join(base_dir, name + "_c")
...
def _gather_indirect(short_name, base_dir, name):
    ...
    for _name in names:
        if short_name.endswith(_name):
            target = _name
            break
        elif short_name.startswith(_name):
            target = _name
            break
...
def _zip_directory(path, relative, file):
    ...
    for entry in entries:
        ...
        if os.path.isdir(_path):
            _zip_directory(_path, _relative, file)
        else:
            file.write(_path, _relative)
